...
@section('content')
<div class="max-w-4xl">
    <form action="{{ route('admin.users.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
            ...
            <div>
                ...
                <select name="role" class="w-full border border-gray-200 px-4 py-3 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white" required>
                    @foreach($roles as $role)
                        <option value="{{ $role->name }}" @selected(old('role', 'Author/Reporter') === $role->name)>{{ $role->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold">Create User</button>
        </div>
    </form>
</div>
@endsection
